...
| **Acceptance telemetry pipeline** (capture YAML edits, build dashboards) | ~40 hours (platform eng) | ~5 hours (dashboard tuning) |
| **Pattern composition validator** (static analyzer for YAML compatibility) | ~40 hours (platform eng) | ~5 hours (rule updates) |
| **Vertex AI Search** (2 data stores: skills, tools — pattern store already operational) | ~20 hours (platform eng) | ~5 hours (catalog updates) |
| **Spec Kit preset** (templates, commands, memory files) | ~20 hours (EA team) | ~5 hours (refinements) |
| **FNOL Pilot** (end-to-end validation) | ~40 hours (pilot team) | — |
...
| **Phase 1 total (1st LOB)** | **~790 hours / ~$79K** | **~125 hours/quarter / ~$12.5K** |
...
| **Break-even (1st LOB)** | Platform cost recovered after 2nd use case. Each additional LOB pays back its onboarding cost in the 1st use case. | |

**Note on pattern authoring:** Pattern documentation is generated by an existing **Pattern Factory** capability (an internal agentic system that produces draft pattern documentation and reference implementations in minutes for EA review). A continuous ingestion pipeline embeds approved patterns into Vertex AI Search. AgentCatalyst consumes this catalog as-is — pattern authoring is not part of AgentCatalyst's cost.
