...
def _token(sub="sarah@company.com", groups=("SA-GROUP",)):
    return jwt.encode({"sub": sub, "name": "S", "aud": CFG.audience, "iss": CFG.issuer,
                       "exp": int(time.time()) + 3600, "scp": "sdlc-accelerators.mcp",
                       "groups": list(groups)}, _PRIV, algorithm="RS256")
...
def _server():
    spec = importlib.util.spec_from_file_location("bf_server", ROOT / "src/brownfield/server/app.py")
    ...
    rows = json.loads((ROOT / "examples/vsphere-mpa-aws-spa/inputs/substitution-table.json").read_text())["rows"]
    m._SUBSTITUTION_ROWS = [SubstitutionRow(**r) for r in rows]
    m._RECOMMEND_FN = lambda s: {"pattern_ref": s["transition_pattern_ref"], "confidence": 0.9,
...
def test_owner_isolation_on_result():
    m = _server()
    start = m.blueprint_start((REF / "spec.md").read_text(), (REF / "plan.md").read_text(),
...
- **Data volume + SLA:**
"""
    start = m.blueprint_start(bad_spec, "### Integration: INT-001 — x\n- **R-factor:** refactor\n",
                              authorization_header="Bearer " + _token())
    assert start["status"] == "failed"
    status = m.blueprint_status(start["taskId"], authorization_header="Bearer " + _token())
    assert status["status"] == "failed"


def test_non_sa_group_forbidden():
    m = _server()
    r = m.blueprint_start("spec", "plan", authorization_header="Bearer " + _token(groups=("other",)))
