...
SUB = {"integration_id": "INT-004", "r_factor": "refactor",
       "target_tokens": ["aws-sqs"], "transition_pattern_ref": "PAT-T-007-dual-publish-mq-sqs"}


def test_injected_recommend_fn_takes_precedence():
    out = ra.recommend_for_integration(SUB, recommend_fn=lambda s: {"pattern_ref": "X", "confidence": 0.9})
...
def test_degrades_to_review_when_provider_unavailable(monkeypatch):
    ...
    assert out["pattern_ref"] == "PAT-T-007-dual-publish-mq-sqs"   # falls back to the substitution ref


def test_live_provider_selects_with_confidence(monkeypatch):
    provider = ra._load_provider()
    monkeypatch.setattr(provider, "available", lambda: (True, ""))
    monkeypatch.setattr(provider, "invoke",
                        lambda sp, um: {"pattern_ref": "PAT-T-007", "confidence": 0.82, "rationale": "best fit"})
    out = ra.recommend_for_integration(SUB)
...
def test_low_confidence_flags_review(monkeypatch):
    provider = ra._load_provider()
    monkeypatch.setattr(provider, "available", lambda: (True, ""))
    monkeypatch.setattr(provider, "invoke", lambda sp, um: {"pattern_ref": "P", "confidence": 0.4})
    out = ra.recommend_for_integration(SUB)
...
def test_prompt_is_bound_verbatim(monkeypatch):
    ...
    monkeypatch.setattr(provider, "available", lambda: (True, ""))
    monkeypatch.setattr(provider, "invoke",
                        lambda sp, um: captured.update(prompt=sp) or {"pattern_ref": "P", "confidence": 0.9})
...
def test_pipeline_uses_live_reasoning_by_default(monkeypatch):
    ...
    rows = [SubstitutionRow(**r) for r in json.loads((ref / "substitution-table.json").read_text())["rows"]]

    provider = ra._load_provider()
    monkeypatch.setattr(provider, "available", lambda: (True, ""))
    monkeypatch.setattr(provider, "invoke", lambda sp, um: {"pattern_ref": "LLM-PICK", "confidence": 0.9})

    result = run_brownfield_pipeline((ref / "spec.md").read_text(), (ref / "plan.md").read_text(),
                                     rows, adr_rules=[])   # NO recommend_fn -> live reasoning path
