...
from brownfield.adr_compliance_check import AdrRule, adr_compliance_check, validate_rule_tests
...
def test_validate_spec_reference_case_passes():
    ...
    assert report.phase_assignment_preview["phase_1"] == ["INT-003"]  # read-only -> Phase 1
...
def _ref_integrations():
    ...
    return [{"integration_id": it.integration_id, "source_tech": it.get("technology + version"),
             "r_factor": plan[it.integration_id].r_factor, "context": plan[it.integration_id].context}
...
def test_substitution_resolves_reference_case():
    ...
    mq = next(s for s in result["tech_substitutions"] if s["integration_id"] == "INT-004")
...
def test_substitution_token_normalization():
    ...
    result = map_current_to_target(_ref_integrations(), _ref_rows())
    apic = next(s for s in result["tech_substitutions"] if s["integration_id"] == "INT-003")
...
def test_substitution_unresolved_errors():
    import pytest
    rows = _ref_rows()
    ints = [{"integration_id": "INT-099", "source_tech": "COBOL on z/OS", "r_factor": "rehost", "context": {}}]
...
def test_substitution_most_specific_wins():
    rows = [
        ...
        SubstitutionRow(["ibm-mq"], "refactor", {"messaging_pattern": "point-to-point"}, ["sqs-p2p"], priority=1),
    ]
    ints = [{"integration_id": "I1", "source_tech": "IBM MQ 9.1", "r_factor": "refactor",
             "context": {"messaging_pattern": "point-to-point"}}]
    result = map_current_to_target(ints, rows)
    assert result["tech_substitutions"][0]["target_tokens"] == ["sqs-p2p"]  # more specific row wins
...
def test_substitution_tie_flags_review():
    ...
    ints = [{"integration_id": "I1", "source_tech": "IBM MQ", "r_factor": "refactor", "context": {}}]
    result = map_current_to_target(ints, rows)
...
def test_predicate_documented_examples():
    assert evaluate("target_tech == 'aurora-mysql' AND data_size_gb > 10000",
                    {"target_tech": "aurora-mysql", "data_size_gb": 15000}) is True
    assert evaluate("cross_cloud_egress AND target_gateway != 'apigee'",
                    {"cross_cloud_egress": True, "target_gateway": "apigee"}) is False
...
def _rule(adr, pred, action):
    return AdrRule(adr_id=adr, key={"source_tech": "*", "target_tech": "*",
                                    "functional_category": "*", "r_factor": "*"},
...
def test_adr_reject_dominates():
    ...
    res = adr_compliance_check("INT-1", {"source_tech": "x", "target_tech": "y",
                                         "functional_category": "z", "r_factor": "refactor"},
                               {"data_size_gb": 20000, "criticality": "tier1"}, rules)
...
def test_validate_rule_tests_requires_three_each():
    import pytest
    rule = AdrRule("ADR-X", {}, "data_size_gb > 100", "REJECT",
