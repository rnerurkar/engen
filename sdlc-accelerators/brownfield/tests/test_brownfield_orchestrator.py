...
def test_orchestrator_five_stages_with_adr_gate_preserved():
    ...
    assert names == ["brownfield_migration_readiness", "brownfield_substitution",
                     "brownfield_pattern_recommender", "brownfield_adr_compliance",
...
def test_substitution_and_assembly_are_deterministic():
    ...
    assert not isinstance(by_name["brownfield_substitution"], LlmAgent)     # no LLM in substitution
    assert not isinstance(by_name["brownfield_assemble_contract"], LlmAgent)  # no LLM in assembly
...
def test_transition_tool_wraps_vertex_and_degrades():
    from brownfield.orchestrator.pattern_search_tool import make_transition_pattern_search_tool
    from clients.vertex_search import VertexSearchClient
    tool = make_transition_pattern_search_tool(VertexSearchClient(_search=lambda ds, q: [{"id": "t1"}]))
...
def test_brownfield_deterministic_steps_have_no_tools():
    ...
    orch = mo.build_orchestrator()
    for name in ("brownfield_migration_readiness", "brownfield_substitution",
                 "brownfield_adr_compliance", "brownfield_assemble_contract"):
