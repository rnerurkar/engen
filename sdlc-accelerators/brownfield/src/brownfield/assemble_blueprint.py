...
class AssembledBrownfieldBlueprint:
    ...
    diagrams: list[Any] = field(default_factory=list)  # [{name, dsl}]


def _phase_blocks(substitutions: Any, plan_by_id: Any) -> Any:
    ...
    for s in substitutions:
        ...
        phases.setdefault(
            phase, {"integration_ids": [], "cross_cloud": False, "coexistence": "none"}
        )
        ...
        if (
            d
            and d.coexistence_window
            and d.coexistence_window.lower() not in ("n/a", "")
        ):
...
def assemble_blueprint(
    spec: Any,
    substitutions: Any,
    pattern_selections: Any,
    attested_adrs: Any,
    plan_decisions: Any,
    readiness_score: Any = 100,
) -> AssembledBrownfieldBlueprint:
    ...
    for it in spec.integrations:
        ...
        lines.append(
            f"- Substitution: {s.get('source_tokens', [])} → {s.get('target_tokens', [])}"
        )
        lines.append(
            f"- Pattern: {sel.get('pattern_ref', 'n/a')} (confidence {sel.get('confidence', 0)})"
        )
        cutover = d.cutover_strategy if d else "?"
        lines.append(
            f"- Transition: {s.get('transition_pattern_ref', 'n/a')}; cutover {cutover}"
        )
    ...
    if any(p["cross_cloud"] for p in phases.values()):
        migration_phases.append(
            {
                "phase": 0,
                "name": "Cross-cloud plumbing (external-team coordination)",
                "integration_ids": [
                    s["integration_id"]
                    for s in substitutions
    ...
    contract = {
        ...
        "staleness_triggers": {
            "adr_store_version": "v1",
            "substitution_table_version": "v1",
            "iac_manifest_version": "v1",
        },
    }
    ...
    diagrams = [
        {
            "name": "component-end-state",
            "dsl": _component_dsl(substitutions, pattern_selections),
        },
        {"name": "sequence-end-state", "dsl": _sequence_end_dsl(spec)},
        {
            "name": "sequence-transition",
            "dsl": _sequence_transition_dsl(migration_phases, plan_by_id),
        },
        {"name": "infrastructure", "dsl": _infra_dsl(substitutions)},
    ]
    return AssembledBrownfieldBlueprint(
        markdown="\n".join(lines), design_contract=contract, diagrams=diagrams
    )


def _component_dsl(subs: Any, sels: Any) -> Any:
...
def _sequence_end_dsl(spec: Any) -> Any:
...
def _sequence_transition_dsl(phases: Any, plan_by_id: Any) -> Any:
    ...
    for ph in phases:
        for iid in ph["integration_ids"]:
            ...
            rows.append(
                f"Phase{ph['phase']} -> {iid}: {d.cutover_strategy if d else ''} {suffix}"
            )
    return "\n".join(rows)


def _infra_dsl(subs: Any) -> Any:
