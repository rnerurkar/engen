...
class MigrationReadinessBlocked(Exception):
    def __init__(self, report):
        self.report = report
        blockers = [f"{ir.integration_id}:{s.signal}" for ir in report.per_integration
                    for s in ir.signals if s.status == BLOCK]
        super().__init__(f"Migration readiness BLOCK (score {report.score}): {blockers}")


def run_brownfield_pipeline(spec_md: str, plan_md: str, substitution_rows: list,
                            adr_rules: list, recommend_fn=None, pattern_candidates=None) -> dict:
    ...
    integrations = [{"integration_id": it.integration_id, "source_tech": it.get("technology + version"),
    ...
    for s in substitutions:
        sel = recommend_for_integration(s, candidates=pattern_candidates, recommend_fn=recommend_fn)
    ...
    # Tool 4: assemble
    assembled = assemble_blueprint(spec, substitutions, pattern_selections, attested_adrs,
                                   plan, readiness.score)
    return {
        "blueprint_md": assembled.markdown,
        "design_contract": assembled.design_contract,
        "diagrams": assembled.diagrams,
        "readiness": {"score": readiness.score, "overall": readiness.overall,
