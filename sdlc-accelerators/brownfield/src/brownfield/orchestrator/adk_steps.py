...
class MigrationReadinessStep(BaseAgent):
    ...
    async def _run_async_impl(
        ...
        if report.overall == BLOCK:
            ...
            yield _text_event(
                self.name,
                f"migration_readiness: BLOCK (score {report.score}) — halting",
            )
            return
        yield _text_event(
            self.name, f"migration_readiness: {report.overall} (score {report.score})"
        )
    ...
    async def _run_async_impl(
        ...
        for s in state.get("substitutions", []):
            ...
            res = adr_compliance_check(
                s["integration_id"], key, dict(s.get("context_matched", {})), adr_rules
            )
            attested.extend(res.attested_adrs)
        state["attested_adrs"] = attested
        yield _text_event(
            self.name, "adr_compliance_check: complete (deterministic predicate DSL)"
        )
...
class AssembleContractStep(BaseAgent):
    ...
    async def _run_async_impl(
        ...
        for s in substitutions:
            sel = (
                recommendation.get(s["integration_id"], {})
                if isinstance(recommendation, dict)
                else {}
            )
            pattern_selections.append(
                {
                    "integration_id": s["integration_id"],
                    "pattern_ref": sel.get(
                        "pattern_ref", s.get("transition_pattern_ref", "n/a")
                    ),
        ...
        if eraser is not None:
            ...
            for d in diagrams:
                rendered = (
                    eraser.render(d["dsl"]) if hasattr(eraser, "render") else None
                )
