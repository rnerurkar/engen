...
def build_recommender_agent(
    model: str = "gemini-2.5-pro", instruction: str = "", search_client: Any = None
) -> Any:
...
def build_orchestrator(
    ...
    recommender = build_recommender_agent(
        model=model, instruction=instruction, search_client=search_client
    )
    return SequentialAgent(
        ...
        sub_agents=[
            MigrationReadinessStep(
                name="brownfield_migration_readiness"
            ),  # deterministic
            SubstitutionStep(name="brownfield_substitution"),  # deterministic, NO LLM
            recommender,  # the ONE LLM stage
            AdrComplianceStep(
                name="brownfield_adr_compliance"
            ),  # deterministic gate (preserved)
            AssembleContractStep(
                name="brownfield_assemble_contract"
            ),  # deterministic + Eraser MCP tool
        ],
    )
