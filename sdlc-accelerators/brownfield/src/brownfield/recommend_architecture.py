...
def _build_user_message(substitution: dict, candidates: list) -> str:
...
def recommend_for_integration(substitution: dict, candidates: list | None = None,
                              recommend_fn=None, prompt: str | None = None) -> dict:
    ...
    if recommend_fn is not None:
        return recommend_fn(substitution)

    provider = _load_provider()
    ok, _reason = provider.available()
    if not ok:
        # Degrade gracefully — never fabricate a confident pick.
        return {"pattern_ref": substitution.get("transition_pattern_ref", "n/a"),
                "confidence": 0.0, "requires_review": True}

    sel = provider.invoke(prompt or BROWNFIELD_RECOMMEND_PROMPT,
                          _build_user_message(substitution, candidates))
    confidence = float(sel.get("confidence", 0.0))
    return {
        "pattern_ref": sel.get("pattern_ref", substitution.get("transition_pattern_ref", "n/a")),
