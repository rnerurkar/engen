...
class ContractManifest:
    ...
    diagrams: list = field(default_factory=list)   # [{name, dsl_obj}]
...
class DesignContractStore:
    ...
    def __init__(self, bucket: str = "<BROWNFIELD_CONTRACT_BUCKET>"):
        self.bucket = bucket
    ...
    def write_contract(self, task_id: str, owner_id: str, blueprint_md: str,
                       design_contract: dict, diagrams: list, _gcs_put=None) -> ContractPointer:
        ...
        self._put(prefix + manifest.contract, json.dumps(design_contract, indent=2).encode(), _gcs_put)
        for d in diagrams:
        ...
        ptr = ContractPointer(task_id=task_id, owner_id=owner_id, gcs_prefix=prefix, manifest=manifest)
    ...
    def read_contract(self, task_id: str, _gcs_get=None) -> dict | None:
