...
class AdrRule:
    adr_id: str
    key: dict                  # {source_tech, target_tech, functional_category, r_factor}
    predicate: str
...
class ComplianceResult:
    ...
    attested_adrs: list = field(default_factory=list)
    violations: list = field(default_factory=list)
    flags: list = field(default_factory=list)


def _rule_applies(rule: AdrRule, key: dict) -> bool:
...
def validate_rule_tests(rule: AdrRule) -> None:
    ...
    if len(pos) < 3 or len(neg) < 3:
        raise ValueError(f"{rule.adr_id}: needs >=3 positive and >=3 negative tests "
                         f"(have {len(pos)}/{len(neg)})")
    for t in pos:
        if not evaluate(rule.predicate, t):
            raise ValueError(f"{rule.adr_id}: positive test did not trip predicate: {t}")
    for t in neg:
        if evaluate(rule.predicate, t):
            raise ValueError(f"{rule.adr_id}: negative test tripped predicate: {t}")


def adr_compliance_check(integration_id: str, key: dict, eval_context: dict,
                         rules: list[AdrRule]) -> ComplianceResult:
    ...
    for rule in rules:
        ...
        if not tripped:
            res.attested_adrs.append({"integration_id": integration_id, "adr_id": rule.adr_id, "result": "pass"})
            continue
        if rule.action == "REJECT":
            res.violations.append(rule.adr_id)
            res.attested_adrs.append({"integration_id": integration_id, "adr_id": rule.adr_id, "result": "reject"})
            worst = "reject"
        elif rule.action == "FLAG":
            res.flags.append(rule.adr_id)
            res.attested_adrs.append({"integration_id": integration_id, "adr_id": rule.adr_id, "result": "flag"})
            if worst != "reject":
                worst = "flag"
