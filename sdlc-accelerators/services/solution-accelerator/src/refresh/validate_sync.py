...
def validate_sync(
    blueprint_md: str, drawio_node_labels: set[str], json_doc: dict[str, Any]
) -> StructuralReport:
    ...
    rep.checks.append(
        Check(
            "node_parity",
            "PASS" if json_agents <= drawio_node_labels else "WARN",
    ...
    rep.checks.append(
        Check(
            "name_matching",
            "PASS" if not missing_in_diagram else "WARN",
            (
                "all agent names match"
                if not missing_in_diagram
    ...
    bad = [
        t["name"]
        for t in json_doc.get("tool_bindings", [])
        if t["assigned_to"] not in json_agents
    ]
    rep.checks.append(
        Check(
            "json_consistency",
...
def _adjacency_check(json_doc: dict[str, Any]) -> Check:
    bad = []

    def walk(n: Any, parent_type: Any = None) -> None:
