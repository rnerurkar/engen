...
def _agent_names(json_doc: dict) -> set[str]:
    names = set()
    def walk(n):
        names.add(n["name"])
...
def _tool_assignments(json_doc: dict) -> dict[str, str]:
    """tool_name -> assigned_to, from last-known json."""
    return {t["name"]: t["assigned_to"] for t in json_doc.get("tool_bindings", [])}


def reconcile(md_agent_names: set[str], drawio_node_labels: set[str],
              md_tool_assignments: dict[str, str], drawio_tool_assignments: dict[str, str],
              last_json: dict) -> Reconciliation:
    ...
    for agent in md_added & drawio_added:                # both added same agent
        recon.deltas.append(Delta(DeltaKind.AGREE, agent, detail=f"both added agent '{agent}'"))
    for agent in md_added - drawio_added:                 # only .md added
        recon.deltas.append(Delta(DeltaKind.MD_ONLY, agent, md_value="added", detail=f".md added '{agent}'"))
    for agent in drawio_added - md_added:                 # only diagram added
        recon.deltas.append(Delta(DeltaKind.DRAWIO_ONLY, agent, drawio_value="added",
    ...
    for tool in set(md_tool_assignments) | set(drawio_tool_assignments):
        ...
        if md_to and dr_to and md_to != dr_to:
            recon.deltas.append(Delta(
                DeltaKind.CONFLICT, tool, md_value=md_to, drawio_value=dr_to,
