...
def detect(blueprint_md: str, drawio_files: dict[str, str], hashes_path: str) -> ChangeDetection:
    ...
    drawio_changed = any(sha256(xml) != drawio_last.get(name) for name, xml in drawio_files.items())
    ...
    if md_changed and drawio_changed:
        ...
        case = Case.NONE
    return ChangeDetection(case=case, md_changed=md_changed, drawio_changed=drawio_changed, last_hashes=last)


def write_hashes(hashes_path: str, blueprint_md: str, drawio_files: dict[str, str], json_doc: dict) -> dict:
