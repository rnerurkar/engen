...
FORBIDDEN_ADJACENCY = {
    (
        "ParallelAgent",
        "LoopAgent",
    ),  # LoopAgent cannot nest directly inside ParallelAgent
}


def validate_composition(agent_tree: dict[str, Any]) -> CompositionResult:
    ...
    def walk(node: Any, parent_type: Any = None) -> None:
        ntype = node.get("type")
