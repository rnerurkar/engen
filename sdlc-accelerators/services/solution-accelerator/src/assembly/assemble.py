...
class AssembledBlueprint:
    ...
    )  # [{name, dsl, drawio_xml, png_base64}]


def _selections_to_appblueprint(
    sel: ArchitectureSelections, json_doc: dict[str, Any]
) -> AppBlueprint:
    return AppBlueprint(**json_doc)


def assemble_blueprint(
    sel: ArchitectureSelections,
    spec: str,
    plan: str,
    eraser_mcp: EraserMcpClient | None = None,
) -> AssembledBlueprint:
