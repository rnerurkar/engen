...
class ArtifactManifest:
    ...
    diagrams: list[Any] = field(
        default_factory=list
    )  # [{name, drawio_xml_obj, png_obj}]
...
class BlueprintArtifactStore:
    ...
    def __init__(self, bucket: str = "<BLUEPRINT_BUCKET>") -> None:
        self.bucket = bucket
    ...
    def write_blueprint(
        ...
        self._put(
            prefix + manifest.json, json.dumps(json_doc, indent=2).encode(), _gcs_put
        )
        ...
        for d in diagrams:
            ...
            self._put(
                prefix + drawio_obj, (d.get("drawio_xml") or "").encode(), _gcs_put
            )
            ...
            manifest.diagrams.append(
                {"name": name, "drawio_xml_obj": drawio_obj, "png_obj": png_obj}
            )

        ptr = BlueprintPointer(
    ...
    def read_blueprint(
        ...
        for d in ptr.manifest.diagrams:
            ...
            diagrams.append(
                {
                    "name": d["name"],
                    "drawio_xml": drawio.decode()
                    if isinstance(drawio, bytes)
                    else drawio,
