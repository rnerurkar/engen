...
# Technical Plan — Order Issue Resolution
...
- Database: AlloyDB (order management)
...
## Security
- Data classification: confidential (PII + payment)
- Identity: per-agent Workload Identity
- Proxy: Apigee (one route per tool binding)
- PII/PAN handling: encrypt at rest (CMEK), mask in logs, VPC-SC, Secret Manager
