# app-blueprint.md — BillingCollections-Forms
...
## §1 Executive Summary
Modernize an Oracle Forms billing UI + PL/SQL collections batch into an Angular SPA + Spring Boot services with scheduled jobs on ECS Fargate, retaining the external PCI payment gateway and moving dunning notifications from MQ to SQS.

- **Current host:** vSphere on-prem (DC-East), Oracle Forms + PL/SQL batch, 4 VMs
- **Target:** ECS Fargate microservices + Angular SPA in 966777888999 (billing-prod), us-east-1 (DR us-west-2)
- **Scope:** 4 integrations — INT-001, INT-002, INT-003, INT-004
- **Criticality / Data:** Tier 2 / Confidential (PII + PCI for stored tokens)
...
| Observability | Splunk + Dynatrace + OTel; no PAN in logs; SLO p95 < 3s |
...
- **INT-001 → `billing-collections-billing-ui`** — Rewrite; cutover **Big-bang**.
- **INT-002 → `billing-collections-collections-engine`** — Rewrite; cutover **Dual-run**.
- **INT-003 → `billing-collections-payment-gateway`** — Retain; cutover **Strangler-fig**.
- **INT-004 → `billing-collections-dunning-notifications`** — Refactor; cutover **Dual-publish**.

Cutover patterns in play: Big-bang, Dual-publish, Dual-run, Strangler-fig.

## §6 Component Architecture (PNG)
![TSA component architecture](billing-collections-tsa.drawio.png)

_Editable source: `billing-collections-tsa.drawio.xml` (Draw.io / `hediet.vscode-drawio`). Renders the target ECS Fargate services, CloudFront/ALB edge, Apigee cross-cloud path, and SQS._

## §7 HA/DR Views (PNGs)
![DR topology](billing-collections-dr.drawio.png)
...
- **Target design:** Spring Boot service on Fargate; contract surface unchanged; in-account call.
- **Validation gate:** 5-cycle ledger parity (diff = 0); aging-bucket reconciliation 100%
...
- **Target design:** Spring Boot service on Fargate; contract surface unchanged; in-account call.
- **Validation gate:** PCI scope review pass; contract tests green; settlement reconciliation = expected
...
- **INT-002:** ledger written by one engine per cycle (run-token lock)
- **INT-003:** one caller per transaction; idempotency key reused
...
- **Pipeline:** Jenkins (`terraform plan/apply`) → Harness (`billing-collections` blue-green / canary deploy to ECS Fargate). No direct CLI deploy.
