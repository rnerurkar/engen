# SDLC Accelerators — GA Architecture
...
SDLC Accelerators does NOT deploy agents. It generates code, infrastructure definitions, and CI/CD pipeline files — then commits everything to the developer's GitHub repo. The company's existing Harness pipeline takes it to production (infra provisioning, per-agent Docker build, Agent Engine deploy, and promotion).

![SDLC Accelerators — End-to-End Component Architecture (Greenfield)](SDLC-Accelerators-Architecture-Greenfield.png)

![SDLC Accelerators — Detailed Step-by-Step Architecture (Greenfield)](SDLC-Accelerators-Architecture-Diagram-Detailed.png)

### SDLC Accelerators at a glance

| Activity | Without SDLC Accelerators | With SDLC Accelerators | Improvement |
|---|---|---|---|
| Requirements capture | 3–5 days (meetings + documents) | 2–4 hours (/specify template) | 90% faster |
| Architecture design | 1–2 weeks (manual research) | 1–5 minutes (Solution Accelerator) | 99% faster |
...
2. **AI-advised, human-decided.** The Solution Accelerator recommends; the developer reviews and edits the blueprint. The human is always in control.
...
|---|---|---|
| Per use case savings | **$39K (74%)** | $52.8K without → $13.8K with SDLC Accelerators |
...
| Solution Accelerator MCP Server (LlmAgent + system prompt + RAG + golden dataset) | ~50 hours (platform eng) | ~5 hours/quarter (prompt tuning) |
...
| GCP infrastructure (Solution Accelerator API layer + pipeline on Cloud Run, AlloyDB Task Store, Vertex AI Search, Arize Phoenix) | — | ~$500–1,500/month |

### Per use case cost comparison (both sides use Copilot)

| | Without SDLC Accelerators (Copilot + published standards) | With SDLC Accelerators (Copilot + skills + Solution Accelerator) |
|---|---|---|
| **Build phase** | 446 hrs (requirements 60 + standards learning 24 + architecture 60 + code 100 + prompts 35 + infra/CI/CD/obs/security 82 + testing 60 + deploy 25) | 138 hrs (requirements 60 + spec/plan/biz rules 8 + Solution Accelerator 0.1 + generate 1 + complex domain logic 20 + prompts 25 + testing 15 + PR 8) |
...
Note: Requirements gathering (60 hrs) is identical on both sides. SDLC Accelerators does not reduce the time to gather requirements — it reduces the time to go from requirements to production-ready code.

### Application archetypes — one platform, many application types

SDLC Accelerators achieves archetype-agnosticism through preset-swapping, not meta-skills or signed contracts:

| Archetype | Preset Name | Spec Template | Catalog | Domain Skill | Status |
|---|---|---|---|---|---|
| **Agentic AI** | `sdlc-accelerators` | agent-spec-template.md | 11 ADK patterns | `adk-agents` | **Phase 1 — active** |
| **Microservice** | `sdlc-accelerators-microservice` | service-spec-template.md | Microservice patterns | `springboot-service` / `fastapi` | **Phase 2 — planned** |
| **Data Pipeline** | `sdlc-accelerators-pipeline` | pipeline-spec-template.md | ETL/ELT patterns | `beam` / `dataflow` | **Phase 3 — planned** |
| **API-First** | `sdlc-accelerators-api` | api-spec-template.md | API patterns | `openapi` / `graphql` | **Phase 4 — planned** |
...
Every GCP service used by SDLC Accelerators is Generally Available with SLA backing. No preview APIs. No preview services. Deployable in any GCP project, including locked-down enterprise environments.

---

## End-to-end thread (read this first)

![Greenfield full lifecycle — build, govern, deploy, operate](greenfield-10-step-flow.png)
...
- **`app-blueprint.md`** — the PRIMARY artifact. Human-readable structured markdown organized in TWO parts:
  - **§1-§9: Governance Sections** — What Governance Guardian assesses. §1 Application Overview, §2 Component Topology Diagram, §3 Architecture Patterns, §4 Tech Stack, §5 DevSecOps Stack, §6 HA/DR Guidance, §7 HA/DR Diagrams, §8 Architecture Decision Log, §9 NFRs. The developer actively edits all 9 sections — every section is genuinely editable.
  - **Technical config** (agent topology, tool bindings, business rules, infra modules, CI/CD, screening, eval) is derived directly into `app-blueprint.json` by `assemble_blueprint` from .md + spec.md + plan.md. Not stored in the .md — no pseudo-editable sections that get overwritten.
- **`app-blueprint.json`** — the DERIVED artifact. Machine-readable JSON derived from app-blueprint.md (§1-§9) + spec.md + plan.md by assemble_blueprint. Consumed by `/accelerator.generate` for deterministic code generation. **Never edit this file directly** — it is regenerated from `.md` by `/accelerator.refresh` (or auto-regenerated before `/accelerator.assess` and `/accelerator.generate`).
- **`diagrams/*.drawio.xml`** — editable diagram sources. Open in Draw.io VSCode extension to edit visually.
- **`diagrams/*.png`** — rendered from the Eraser DSL by the **Eraser MCP server** (Solution Accelerator sends DSL, receives `.drawio.xml` + `.png` synchronously). Referenced in `.md` via `![](diagrams/...)`. Regenerated by `/accelerator.refresh`.
- **Diagram files** — `.drawio.xml` (editable in Draw.io VSCode extension) + `.png` (rendered by the **Eraser MCP server**, inline in markdown)
...
She reviews the markdown in her editor — the component diagram renders inline in VSCode's markdown preview, all tables are readable. The Solution Accelerator assigned Cloud SQL to the wrong agent — she edits the table directly, changing `assigned_to: extract_details` to `assigned_to: fnol_coordinator`. She saves. Her coding agent calls `validate_composition` via MCP — a deterministic check that her edited pattern tree is valid (e.g., LoopAgent cannot nest inside ParallelAgent). It passes. Then `assemble_blueprint` finalizes the markdown **and regenerates `app-blueprint.json`** from the edited sections (the JSON is always derived from the `.md` — never edited directly). If she needs to edit the component diagram, she opens the `.drawio.xml` file in the Draw.io VSCode extension — adds a fraud-check agent box, draws a connection to the fraud-db MCP server, saves. She types `/accelerator.refresh`. The refresh detects that only the .drawio.xml changed (Case B), parses the new node and edge from the diagram, updates `app-blueprint.json` with the new agent and tool binding, and uses the Solution Accelerator LLM to update §2's narrative and mermaid sequence in the .md to reflect the addition — all automatically. She can also edit in the opposite direction: add an agent in the .md prose, and `/accelerator.refresh` regenerates the .drawio.xml to match (Case A). If she edited both files, refresh detects conflicts and asks her to resolve them (Case C). Updated files are picked up automatically on her next `/accelerator.assess` run.

### Spec Signal Validation (Quality Gate in Solution Accelerator)

Before the RAG pipeline runs, the Solution Accelerator validates that the spec contains the signals needed for high-quality architecture retrieval. This is the first step inside `blueprint_start` — a quality gate, not a reasoning step.
...
| §2 Workflow | Dynamic tool-use signal ("decide which tool", "based on the result", "reason about the next action") when steps lack fixed ordering | Distinguishes **ReAct** from Sequential; a tool-driven agent otherwise mis-types as a fixed pipeline | Ordering words OR an explicit ReAct signal | Tool-heavy workflow with neither | — |
| §2 Workflow | Iteration + threshold pairing ("refine/repeat until \<score/condition\>") | Confirms **Loop**, and whether a **Critic** belongs inside it | "until \<threshold\>" stated; if a score is referenced, a validate/score step named | "until" with no threshold, or a score with no evaluator step | — |
| §2 Workflow | Oversight signal ("oversee", "monitor", "quality-gate the agents") | Separates **Supervisor** from **Coordinator** | Oversight stated explicitly | Multiple agents but oversight role implied, not stated | — |
| §2 / §7 | Validation/scoring signal ("validate", "score", "evaluate", "grade") | Determines **Critic / Evaluator** | An evaluation verb present where an output is judged | Evaluation step described without an evaluation verb | — |
| §1 / §2 | Event-initiation signal ("when \<event\>", "triggered by", "on message") | Determines **Event-Driven** (async trigger) vs synchronous request | Trigger stated where initiation is async | Async start implied but not flagged | — |
| §4 Data Sources | Retrieval-corpus signal ("search the document/knowledge corpus") distinct from a transactional store | Determines **RAG** vs transactional MCP | Phrased as document/knowledge retrieval | "search" used ambiguously over a transactional store | — |
...
Before generating code, she runs the governance check. She edits §2 (adds a fraud-check agent to the topology) and opens `component-architecture.drawio.xml` in Draw.io to add the box. She types `/accelerator.refresh` — it detects that both .md and .drawio.xml changed (Case C: she edited §2 in the .md AND added the box in Draw.io). It auto-merges the consistent changes, and since both agree on the fraud-check-agent addition, no conflict. It regenerates .json (from synced .md + spec.md + plan.md) + .drawio.xml + .png, with the .md §2 narrative updated to include the new agent. **Skip-refresh safety:** If `/accelerator.assess` detects that `app-blueprint.md` or any `.drawio.xml` file has been modified since the last `/accelerator.refresh` (by comparing file timestamps with `.json` timestamp), it auto-triggers a lightweight refresh as Step 0 — validating .md structure, checking .md↔.drawio consistency, and regenerating `.json` + `.png`. The developer sees: "Stale .json detected — auto-refreshing before assessment..." This means `/accelerator.refresh` is OPTIONAL as a standalone command (for detailed validation feedback) but AUTOMATIC before assessment and code generation. The developer can never accidentally assess or generate from stale files.

She types `/accelerator.assess`. The coding agent reads `app-blueprint.md` (NOT `app-blueprint.json` — the Governance Guardian assesses the human-readable architecture, not the machine-readable JSON) and extracts all 9 governance sections — §1 Application Overview, §2 Component Topology Diagram (+ PNG from `![...]()` reference), §3 Architecture Patterns, §4 Tech Stack, §5 DevSecOps Stack, §6 HA/DR Guidance, §7 HA/DR Lifecycle Diagrams (+ PNG from `![...]()` reference), §8 Architecture Decision Log, §9 NFRs — packages them as a **solution_package** and sends them to the **Governance Guardian MCP Server** using the same async pattern as the Solution Accelerator (`assess_start` → poll `assess_status` → `assess_result`). **PDF translation for the Eraser assessment engine:** the Governance Guardian's assessment engine (Eraser MCP server) expects a **PDF**, not Markdown. So `assess_start` first converts `app-blueprint.md` into a PDF — each §N section becomes a section in the PDF, and every PNG referenced via `![...]()` inside a section is embedded under that same section (the §1–§9 template structure is preserved). The blueprint PDF is sent to the Eraser MCP server, which returns its **findings and scorecard as a PDF**. The Governance Guardian converts that findings PDF back into a **Markdown** document — findings grouped into **Critical / High / Medium / Low** categories plus the scorecard — which `assess_result` returns to the IDE for the developer to act on. The `app-blueprint.json` file in the workspace is not read, not sent, and not modified during governance assessment — it exists solely for `/accelerator.generate` to consume later. While the EA assessment engine evaluates her solution (a black box to SDLC Accelerators — the EA office owns all the assessment logic), she sees progress in the Chat pane: "Evaluating architecture compliance...", "Checking pattern adherence...", "Scoring HA/DR readiness...".

The assessment returns a scorecard and findings. One showstopper: her Aurora PostgreSQL has no cross-region DR strategy, violating ADR-205. Two non-critical findings: WAF rules not using the enterprise managed rule group, and Angular 17 not yet on the approved tech radar. She fixes the showstopper — adds Aurora Global Database with a us-west-2 read replica to her Terraform and updates the HA/DR view in her drawio. She runs `/accelerator.assess` again. This time: no showstoppers, score 88/100. The two remaining findings are flagged as acceptable tech debt.

She types `/accelerator.generate`. **Server-side governance gate:** Before the generation pipeline runs, the coding agent calls the Governance Guardian's `verify_generation_gate(taskId)`. The Governance Guardian does NOT trust a client-supplied verdict — it reads the persisted findings server-side. At `assess_result` time, the findings Markdown was written to a **GCS bucket** with a pointer row recorded in **AlloyDB** (task_id, owner_id, gcs_uri, has_blocking). The gate reads `findings.md` back via that pointer and checks the severity categories:
- **If any Critical or High finding remains**, the gate returns `{ signal: "stop" }` with a message instructing the developer to resolve those findings, then run `/accelerator.refresh` to validate that the diagram (`.drawio.xml`) and `app-blueprint.md` are back in sync, then re-run `/accelerator.assess`. Generation stays blocked until no Critical or High findings remain.
- **If only Medium and Low findings remain**, the gate returns `{ signal: "resume" }` and writes **one tech-debt JSON object per remaining finding** to a GCS tech-debt bucket (each tagged `status: accepted` with a `TD-<year>-<task>-NNN` id, severity, section, owner). 

In her case, after fixing the showstopper, only the two Medium/Low findings remain, so the gate resumes, records the two tech-debt JSON objects, and the coding agent reports: "Governance gate passed. Tech debt recorded. Generating code..." and proceeds.
...
She commits. The pre-commit hook runs — 5 evaluation sets execute in under 60 seconds via the Vertex AI Evaluation SDK. All metrics pass. She pushes and opens a PR. Her team reviews it — the generated code looks familiar because every SDLC Accelerators project follows the same company patterns.

After the PR is merged, the Harness pipeline runs end to end: it provisions infrastructure (`terraform apply`, approval-gated), builds a Docker image per agent, and deploys to Agent Engine (publishing the proxy API to Apigee Gateway). It runs the 3-phase evaluation pipeline: Phase A (Arize quality gates), Phase B (AutoSxS baseline comparison against the golden dataset), Phase C (2 edge cases routed to HITL triage where a reviewer approves them). After evaluation passes, Harness promotes through Non-Prod → Pre-Prod (canary at 10%) → Production (progressive rollout) — and in each environment it registers the agent as an API Product in API Hub for that environment. If anything breaks, Harness rolls back automatically.
...
**Total time from "I need an FNOL agent" to generated code committed to GitHub: under 2 hours.** The remaining 2–4 hours are spent reviewing generated business logic, writing system prompts, curating eval datasets, and adding edge cases. Without SDLC Accelerators, this entire process takes 7–8 weeks.
...
| Runtime (API layer) | Cloud Run Service (GA) — hosts the MCP API layer for Solution Accelerator |
| Runtime (Pipeline) | Cloud Run Jobs (GA) — runs the Solution Accelerator LlmAgent pipeline (no timeout) |
...
| Spec Workflow | GitHub Spec Kit with SDLC Accelerators preset (archetype-specific) |
| Solution Accelerator | LlmAgent exposed as MCP Server. API layer on Cloud Run Service (async via MCP Tasks). Pipeline on Cloud Run Jobs. Task state in AlloyDB. Enqueue via Cloud Tasks. |
...
| CI/CD | Harness (single pipeline: infra provisioning + per-agent Docker build + Agent Engine deploy + 3-phase EvalOps + per-env promotion & API Hub registration) |
...
Layer 2 — ARCHITECTURE ADVISORY Solution Accelerator MCP Server → app-blueprint.md
Layer 3 — SKILL-GUIDED GEN     /accelerator.generate → complete project
Layer 4 — COMPANY CI/CD        Harness (infra + build + deploy + EvalOps + promote)
...
The developer installs the SDLC Accelerators preset and runs `/specify` + `/plan`. The preset is archetype-specific — an agentic preset captures workflow ordering words and agent boundaries, a microservice preset captures API contracts and data models.
...
|---|---|---|
| Business Problem | Context and value proposition | Informs Solution Accelerator recommendations |
| Workflow | Step-by-step with ordering words | Solution Accelerator maps to patterns |
| Data Sources | Data systems with workload types | Solution Accelerator assigns MCP connections |
| External Integrations | Partner services | Solution Accelerator assigns A2A or FunctionTool wrappers |
| Internal Capabilities | Proprietary logic | Solution Accelerator flags as FunctionTool implementations |
...
### Layer 2 — Architecture Advisory (Solution Accelerator MCP Server)

![Solution Accelerator — Async Internal Architecture (MCP Tasks)](solution-accelerator-components.png)

The Solution Accelerator is an LlmAgent running on Cloud Run, **exposed as an MCP Server**. The coding agent connects via MCP protocol — this is the only universally compatible method (GitHub Copilot cannot make HTTP calls or run shell commands, but all major coding agents support MCP).

**Async invocation via MCP Tasks:** VS Code Copilot enforces a hard 10–15 second timeout on synchronous MCP tool calls. The Solution Accelerator's internal pipeline (2 RAG queries + API Hub discovery + LLM reasoning + validation + assembly) takes 15–60 seconds depending on spec complexity. A synchronous call would be killed by Copilot before it completes. The Solution Accelerator therefore uses the **MCP Tasks** async primitive (spec revision 2025-11-25): the coding agent starts a background task, polls for progress, and retrieves the result when complete. Each individual MCP call completes in under 2 seconds — well within any coding agent's timeout window.
...
| `recordTechDebt(findings)` | **SYNCHRONOUS** | < 1 second | Records non-showstopper findings as accepted tech debt; returns resume/stop signal. |
| `verify_generation_gate(taskId)` | **SYNCHRONOUS** | < 1 second | **Server-side generate gate.** Reads the persisted `findings.md` from GCS via its AlloyDB pointer (owner_id-isolated). If any **Critical or High** finding remains → `{signal: stop, message}` telling the developer to resolve them and run `/accelerator.refresh` to re-validate `.md`↔`.drawio` sync, then re-assess. If only **Medium/Low** remain → writes one tech-debt JSON per finding to the GCS tech-debt bucket and returns `{signal: resume, tech_debt_uris}`. |
...
| `assemble_blueprint(selections, spec, plan)` | **DETERMINISTIC** | < 1 second | Rebuilds final `app-blueprint.md` from validated selections + **regenerates `app-blueprint.json`** from the `.md` sections (machine-readable derived artifact). Constructs the **Eraser DSL** from topology and **sends it to the Eraser MCP server**, which renders `.drawio.xml` + `.png` **synchronously** (one call per diagram). No LLM involved. Auto-called by `/accelerator.generate` if `.md` was edited since last assembly. |
...
**Solution Accelerator versioning:**
...
This enables reproducibility: if a developer needs to understand why a recommendation was made, the platform team can re-invoke the same Solution Accelerator version with the same spec. The Operations Runbook (Section 9) covers the deployment procedure for new versions, including maintaining 2 versions in production for rollback.

**Internal to the MCP Server (NOT exposed to the coding agent):**

| Internal Component | Purpose |
|---|---|
| Solution Accelerator LlmAgent | RAG + LLM reasoning guided by company system prompt |
...
| Company system prompt | Curated best practices, constraints, preferences. → *See Developer Guide "System Prompt Template — Greenfield Solution Accelerator" for the full system prompt.* |
...
**MCP protocol version:** The Solution Accelerator MCP Server implements **MCP protocol version 2025-11-25** (which introduced the Tasks primitive for async operations). Coding agent compatibility:
...
For coding agents without native MCP Tasks support (most IDE-based agents), the `/accelerator.blueprint` prompt file drives the start → poll → result loop. The LLM naturally handles the polling — each tool call is a fast round-trip.
...
**`/accelerator.blueprint` command flow (async via MCP Tasks):**
...
2. Background pipeline starts on Cloud Run Jobs (no timeout constraint):
   - Runs Solution Accelerator LlmAgent internally (RAG search → LLM reasoning → recommendations)
...
**Prompt-file orchestration:** The `/accelerator.blueprint` prompt file drives the start → poll → result loop without custom client code. The LLM naturally handles the polling — each tool call is a fast round-trip within any coding agent's timeout window. The developer sees progress messages in the Chat pane throughout.

**Offline / disconnected fallback:**

If the Solution Accelerator MCP Server is unreachable (VPN down, server maintenance, network issue), the developer is NOT blocked. Two fallback paths exist:

1. **Manual YAML authoring:** The developer writes `app-blueprint.md` manually using the schema reference (see Appendix A.10 for a complete example). The coding agent can still run `/accelerator.generate` against a hand-written blueprint — it only needs `app-blueprint.md`, not the MCP Server.
...
| Pattern | ADK Class | When Solution Accelerator selects it |
|---|---|---|
...
| **C (recommended)** | Hybrid. Keep template patterns for simple compositions. Add graph-based for complex. Solution Accelerator decides which layer based on spec complexity. | Medium | Future phase |
...
The following sequence diagram shows the complete developer workflow across all 4 phases: `/accelerator.blueprint` (async MCP Tasks), HITL Edit + `/accelerator.refresh` (bidirectional sync with Case A/B/C), `/accelerator.assess` (Governance Guardian), and `/accelerator.generate` (code generation).
...
sequenceDiagram
    ...
    alt Case A: Only .md changed
        ...
        BA->>ER: Generate Eraser DSL from .json, render(dsl) → .drawio.xml + .png
    ...
    BA->>BA: Regenerate .json + .accelerator-hashes
...
### Solution Accelerator MCP Server — Security

The Solution Accelerator MCP Server receives spec.md content that may contain proprietary business rules, competitive intelligence, partner names, and regulatory details. The following security controls are required:

**Authentication — OAuth 2.1 with Entra ID:**

Both the Solution Accelerator and Governance Guardian MCP Servers require OAuth 2.1 authentication via the company's **Microsoft Entra ID** (formerly Azure AD) identity provider. The coding agent authenticates the developer once, then attaches the access token to every MCP tool call. The token is validated by both MCP Servers — a developer authenticated for the Solution Accelerator does NOT need to re-authenticate for the Governance Guardian (same token, same IdP, same audience scope).

The following sequence diagram shows the complete authentication flow from the developer's first `/accelerator.blueprint` command through token acquisition to authenticated MCP tool calls on both servers:
...
sequenceDiagram
    ...
    Dev->>Entra: Browser redirect to /authorize<br>response_type=code, client_id, scope=sdlc-accelerators.mcp<br>code_challenge (PKCE S256)
...
| **Single audience scope** (`sdlc-accelerators.mcp`) | Both Solution Accelerator and Governance Guardian share the same scope — one token works for both servers. The developer authenticates once. |
| **Solution Architect group gating** | Beyond a valid token, tool access requires membership in the **Solution Architect** Entra AD group. Each MCP Server checks the `groups` claim of the validated JWT against the group's object ID; a developer with a valid token but outside the group receives **403 (insufficient_scope)** and cannot invoke any tool. This authorizes *who* may use the platform, on top of authenticating *that* they are who they claim. |
...
| **JWT validation at each MCP Server** | Each server independently validates, on every call: (1) JWT signature against Entra ID's JWKS endpoint (cached, ~1ms), (2) issuer + audience, (3) required scope `sdlc-accelerators.mcp`, and (4) **Solution Architect group membership** (`groups` claim). No shared session state between servers. Any failure returns 401/403 with a `WWW-Authenticate` challenge carrying the Entra authorize endpoint, which drives the IDE back through the OAuth flow. |
...
**Blueprint Artifact Store (GCS + AlloyDB pointer):**

`assemble_blueprint` produces a set of artifacts: `app-blueprint.md`, `app-blueprint.json`, and per-diagram `.drawio.xml` + `.png` (rendered synchronously by the Eraser MCP server). Rather than carry these (including binary PNGs) in the async task record, the Solution Accelerator writes the full set to a **GCS bucket** under a per-task prefix (`gs://<bucket>/blueprints/<owner_id>/<task_id>/`) and records **one pointer row in AlloyDB** (`task_id`, `owner_id`, `gcs_prefix`, artifact manifest). `blueprint_result` reads the pointer by `task_id` (owner_id-isolated via Row-Level Security), fetches the artifacts from GCS, and returns them to the IDE combined (md + json inline, diagrams as base64). This mirrors the Governance Guardian's findings store (findings.md in GCS + AlloyDB pointer): AlloyDB holds only small pointer rows; binary PNGs live in object storage where they belong. The same `task_id` that the IDE polls is the lookup key for the artifacts.

**Authorization — Solution Architect group gating:**

Authentication proves *who* the developer is; authorization decides *whether* they may use the platform. The platform restricts MCP tool access to members of the **Solution Architect** Microsoft Entra ID group:

- The IDE app registration is configured to emit the `groups` claim (object IDs) in the access token. For tenants with many groups, the group-overage path (Graph `getMemberObjects`) is used; otherwise the `groups` claim is carried directly in the JWT.
- Each MCP Server is configured with the Solution Architect group's object ID (`SOLUTION_ARCHITECT_GROUP_ID`). On every tool call, after signature/issuer/audience/scope validation, the server checks that the group ID is present in the token's `groups` claim.
- A developer who authenticates successfully but is **not** in the Solution Architect group receives **403 Forbidden** (`error="insufficient_scope"`, description "Access restricted to the Solution Architect group") and cannot invoke any tool on either server.
- Both MCP Servers enforce the same group check independently — there is no path to a tool that bypasses it. Adding or removing a developer's access is a group-membership change in Entra ID; no code or config change is required.

This sits on top of the per-developer tenant isolation (`owner_id` from the `sub` claim): the group claim authorizes access to the platform, while `owner_id` ensures one authorized developer cannot read another's tasks.
...
- When the developer installs the preset (`specify preset add sdlc-accelerators-enterprise`), the connection configuration is installed automatically
- First-time `/accelerator.blueprint` or `/accelerator.assess` triggers the browser-based SSO flow
...
- Queue IAM is scoped to the Solution Accelerator service accounts only — no developer-facing access
...
### Solution Accelerator MCP Server — Capacity and Rate Limiting

The Solution Accelerator uses an async two-component architecture: a lightweight **MCP API layer** (Cloud Run Service) handles the three fast tools, and a **background pipeline** (Cloud Run Jobs) runs the LlmAgent work with no timeout constraint. Each `blueprint_start` triggers a pipeline run that takes 15–60 seconds (2 RAG queries + API Hub discovery + LLM reasoning + validation + assembly) and costs ~$0.01 (Vertex AI Search + Gemini API tokens).
...
**Total monthly Solution Accelerator cost at expected usage: $15–30/month.** The exec summary's $500–1,500/month GCP infrastructure estimate covers the full platform, not just the Solution Accelerator. The reconciliation:

| Component | Monthly cost (Year 1) |
|---|---|
| Solution Accelerator (API layer + pipeline + AlloyDB + Gemini + Vertex AI Search) | $15–30 |
...
| Cloud Run for deployed agents (runtime, not Solution Accelerator) | $100–400 |
...
The $500–1,500/month estimate in the exec summary is accurate for the full platform. The capacity section above covers only the Solution Accelerator's share.
...
```yaml
# In the Harness pipeline — runs in EACH environment during promotion (non-prod → pre-prod → prod).
# Each environment registers the agent as its own API Product in API Hub for that environment.
- step:
    ...
    command: |
      apihub register \
        ...
        --endpoint=https://fnol.${HARNESS_ENV}.internal/a2a \
        --agent-card=https://fnol.${HARNESS_ENV}.internal/.well-known/agent.json \
        --lifecycle=active \
        --labels=lob:insurance,domain:claims
```

Once registered, future Solution Accelerator runs discover this agent via `discover_integrations()` and can recommend A2A delegation to it — reusing the deployed agent instead of rebuilding duplicate capability. This creates a flywheel: the more agents deployed via SDLC Accelerators, the more agents available for A2A delegation in future projects.
...
The pattern repo is selected by the Solution Accelerator based on two fields: the archetype (agentic, microservice, pipeline, API) and the DR strategy from plan.md (backup-restore, pilot-cold, pilot-ondemand, warm-standby). The service modules are selected based on the tech stack in §4 Application Tech Stack.
...
**GitHub MCP Server role in the flow:** The architecture diagram shows a dashed arrow from Step 4 (code generation) to the GitHub MCP Server. This represents the coding agent reading module repos during generation — it's the only step where the coding agent accesses GitHub directly. The Solution Accelerator accessed GitHub earlier (Step 2) to discover which repos exist; the coding agent accesses the same repos (Step 4) to read the actual module interfaces and generate compliant Terraform.
...
### Layer 4 — Company CI/CD (outside SDLC Accelerators scope)

SDLC Accelerators generates code and pipeline definitions. The company's existing CI/CD executes them.

| Pipeline | Tool | Purpose |
|---|---|---|
| Infrastructure + deploy | Harness | `terraform plan` → `terraform apply` (provisions Agent Engine runtime, Apigee proxy routes, Workload Identity SAs, Cloud SQL, Model Armor, VPC-SC) → per-agent `docker build` → deploy to Agent Engine |
| Application plane | Harness | Deploys agent → runs 3-phase EvalOps → promotes Non-Prod → Pre-Prod (canary) → Production |
| Post-deployment (per env) | Harness | In each environment (non-prod → pre-prod → prod), registers the agent as an API Product in Apigee API Hub for that environment (`type=a2a_agent`, capabilities, Agent Card URL) → enables A2A discovery by future Solution Accelerator runs |
...
**Phoenix tracing scope:** Phoenix tracing is for **generated agents at runtime** — LLM calls, tool calls, and agent delegation within your deployed agent. The Solution Accelerator background pipeline is **not** traced by Phoenix; it is traced via Cloud Logging (structured logs per pipeline stage) and Dynatrace APM (OTel spans for RAG query latency, LLM reasoning time, and end-to-end pipeline duration). Platform engineers troubleshooting Solution Accelerator performance use Dynatrace, not Phoenix. See the Operations Runbook, Section 9 for monitoring details.
...
The `app-blueprint.md` is a **9-section governance document** that serves two roles: (1) human-readable governance review artifact (§1-§9 — what Governance Guardian assesses and what the SA reviews), and (2) source of truth for architectural decisions. Technical config (agent topology, tool bindings, business rules, infra modules, CI/CD, screening) is derived into `app-blueprint.json` by `assemble_blueprint` from .md + spec.md + plan.md — not stored in the .md. Every section in the .md is genuinely editable — no section is auto-regenerated from another section.

**§1-§9: Governance Sections** — what Governance Guardian assesses. Written by the developer with Solution Accelerator guidance. Human-readable narrative + summary tables. Every section is genuinely editable.
...
### Solution Accelerator Output Confidence Assessment

The following confidence levels (0-1 scale) represent how reliably the Solution Accelerator generates each section from `spec.md` + `plan.md` inputs without manual correction. SAs should focus review time on lower-confidence sections.
...
1. **HA/DR grounding (§6, §7):** The Solution Accelerator sources RPO/RTO numbers from `memory/hadr-reference.md` (a curated reference table maintained by platform engineering) rather than relying on LLM training data.

2. **ADL specificity (§8):** The Solution Accelerator's reasoning trace is structured and extractable. Each ADL entry includes the specific spec section, plan choice, or API Hub result that drove the decision.

3. **Diagram simplification (§7):** For HA/DR lifecycle diagrams, the Solution Accelerator generates one consolidated diagram with a state table below it rather than four separate diagrams.
...
| 3 | **Architecture Decision Log** | Decisions + rationale + alternatives considered + ADR compliance | Solution Accelerator RAG reasoning + ADR Store |
...
| 6 | **Component Architecture** | Narrative description + `![Component Diagram](diagrams/component-architecture.png)` | Solution Accelerator + Draw.io render |
| 7 | **HA/DR & Lifecycle Views** | Per-scenario narrative + `![](diagrams/hadr-*.png)` + recovery table (RTO/RPO) | plan.md DR strategy |

#### Technical Config (derived into app-blueprint.json only — not in .md)
...
├── app-blueprint.md                           ← SOURCE: 9-section governance document
...
```json
{
  "markdown": "<full content of app-blueprint.md — 9 sections>",
  "blueprint_json": "<full content of app-blueprint.json — derived from .md + spec.md + plan.md>",
  "diagrams": [
...
### /accelerator.refresh — Structural Validation + File Regeneration

After the developer edits `app-blueprint.md` and/or `diagrams/*.drawio.xml`, they run `/accelerator.refresh` to validate structural integrity and regenerate .json + .png.

**MCP tool:** `refresh(blueprint_md, drawio_files[], spec, plan)` — on the Solution Accelerator MCP Server.

![/accelerator.refresh — Bidirectional Sync Flow](refresh-sync-flow.png)
...
app-blueprint.md (prose, human-readable)
        ↕ LLM-assisted (Solution Accelerator)
app-blueprint.json (structured topology — reconciliation hub)
        ↕ deterministic (Eraser DSL → Eraser MCP server)
...
/accelerator.refresh
  ...
  │   Read .accelerator-hashes → compare against current file timestamps
  ...
  │   ├── Send edited .md + spec.md + plan.md to Solution Accelerator
  │   ├── LLM extracts topology changes from edited prose
  │   ├── Regenerate app-blueprint.json from .md + spec + plan (assemble_blueprint)
  │   ├── Generate Eraser DSL from updated .json topology (deterministic)
  │   ├── Eraser MCP server: render DSL → .drawio.xml + .png (synchronous)
  ...
  │   ├── Send updated .json to Solution Accelerator
  ...
  │   ├── Regenerate .png via Eraser MCP server (submit DSL → fetch .png)
  │   └── Write updated .md
  │
  ...
  │   ├── Diff both against last-known .json (from .accelerator-hashes):
  ...
  └── Step 3: REGENERATE derived files
      ...
      ├── .png: render via Eraser MCP server (submit DSL → fetch .png)
      ├── Update .accelerator-hashes with new SHA-256 values
...
5. **PRESERVE developer overrides.** If the developer changed a tool timeout from 30s to 60s in the .json, the refresh does NOT clobber it. Only NEW topology elements get defaults. This is tracked via the `.accelerator-hashes` diff — changes relative to last-known-good are additions/removals, not replacements.

### Staleness Detection — Skip-Refresh Safety Net

If the developer skips `/accelerator.refresh` and goes directly to `/accelerator.assess` or `/accelerator.generate`, the system detects stale files:

**How it works:** Every time Solution Accelerator generates or refreshes files, it records SHA-256 hashes in a `.accelerator-hashes` file:
...
**On /accelerator.generate:**
1. Same staleness check
2. If .json is stale → **BLOCK with specific error:**
   "❌ app-blueprint.json is stale — .md was modified since last refresh. Run /accelerator.refresh first, or run /accelerator.assess (which auto-refreshes)."
3. /accelerator.generate does NOT auto-refresh — it requires explicit refresh or assess. This is intentional: code generation from stale .json is dangerous, and the developer should consciously validate before generating code.
4. **Server-side governance gate:** after the staleness check, the coding agent calls `verify_generation_gate(taskId)`. The Governance Guardian reads the persisted `findings.md` from GCS (via its AlloyDB pointer) and blocks generation if any **Critical or High** finding remains, returning a message to resolve them and run `/accelerator.refresh` (to re-sync `.md`↔`.drawio`) then re-assess. If only **Medium/Low** remain, it writes one tech-debt JSON per finding to the GCS tech-debt bucket and returns `resume`. The verdict is computed server-side from persisted findings — never trusted from the client.

**Why /accelerator.assess auto-refreshes but /accelerator.generate blocks:**
...
Solution Accelerator generates diagrams during `assemble_blueprint`:
...
3. Renders `.png` via the Eraser MCP server (submit DSL → fetch `.drawio.xml` + `.png`)
...
|---|---|---|
| SDLC Accelerators platform | Platform Engineering | Solution Accelerator MCP Server, Governance Guardian MCP Server, overlay skills, preset catalog, Vertex AI Search data stores |
| Pattern catalog | Enterprise Architecture | Pattern documentation, composition rules, HA/DR views |
| **EA governance standards** | **Enterprise Architecture** | **Governance Guardian assessment logic, scoring rubrics, EA standards (black box to SDLC Accelerators)** |
...
To request new patterns, skills, or tools: submit a PR to the SDLC Accelerators catalog repo. The platform team reviews weekly. To report Solution Accelerator quality issues: file a ticket with the spec.md and the generated blueprint. To report Governance Guardian assessment issues: file a ticket with the solution_package JSON and the findings — the EA office reviews assessment logic. See the Operations Runbook for telemetry-driven quality improvement procedures and the Governance Guardian Architecture Extension for assessment procedures.

---

## What SDLC Accelerators is NOT
...
| 2 | Solution Accelerator MCP Server deployed with OAuth 2.1 | ⬜ |
...
| 14 | `/accelerator.assess` → `/accelerator.generate` flow tested end-to-end (including showstopper block + tech debt resume) | ⬜ |
...
|---|---|---|---|---|
| 1 | Solution Accelerator recommends wrong pattern | Medium | Medium | Confidence scores visible in blueprint. Developer reviews. `validate_composition` catches invalid nesting. Acceptance telemetry tracks accuracy. |
...
| 6 | Governance Guardian becomes bottleneck | Medium | Medium | Assessment is async (non-blocking). SA can skip assessment with `/accelerator.generate --skip-assess` (recorded as policy exception). Tech debt path allows proceeding with non-showstoppers. EA office monitors assessment turnaround via telemetry. |
...
|---|---|---|---|
| **This Architecture Document** | `sdlc-accelerators-architecture-archetype-agnostic.md` | Architects, tech leads | Architectural decisions, layer deep dives, cost model, ROI |
| **SDLC Accelerators Developer Guide** (GA) | `sdlc-accelerators-archetype-agnostic-developer-guide.md` | Developers | Step-by-step walkthroughs, full preset file contents, code examples, spec writing, troubleshooting |
| **SDLC Accelerators Operations Runbook** | `sdlc-accelerators-operations-greenfield_runbook.md` | Platform engineering | Wire-level Vertex AI Search APIs, search quality regression suite, acceptance telemetry, catalog quality engineering, tool lifecycle management, failure modes, escalation matrix, EvalOps operations |
| **Governance Guardian Architecture Extension** | `governance-guardian-architecture.md` | Architects, EA office | `/accelerator.assess` design, async assessment flow, `recordTechDebt` gate in `/accelerator.generate`, Tech Debt Registry, solution package schema, scorecard format |
...
This appendix contains every template file in the `sdlc-accelerators-enterprise` preset. FNOL-specific samples (filled examples) are in the Developer Guide Appendix.
...
├── commands/
│   ├── accelerator.blueprint.md               ← P1: Solution Accelerator call
│   ├── accelerator.assess.md                  ← P2: Governance Guardian call
│   └── accelerator.generate.md                ← P3: Code generation + gov gate
├── skills/
...
→ *app-blueprint.md template (F1) is in `app-blueprint-md-template-and-fnol-example.md` (9-section template + FNOL example).*
...
version: "1.0.0"
description: >
  SDLC Accelerators enterprise agent development accelerator.
  Structured requirements capture, AI-assisted architecture advice
  via Solution Accelerator, and skill-guided code generation.
...
<!-- These ordering words are signals for the Solution Accelerator to select agentic patterns. -->
...
```markdown
---
template: sdlc-accelerators-tasks
version: "2.0"
archetype: agentic
---
...
## Auto-Generated by /accelerator.generate (80-95%)
...
| Agent Workload Identity | `deployment/terraform/identity/*.tf` | app-blueprint.json adk_agent_tree + tool_bindings |
...
### P1 — commands/accelerator.blueprint.md

```markdown
---
command: accelerator.blueprint
description: Send spec and plan to Solution Accelerator for AI-assisted architecture advice
---

# /accelerator.blueprint

## What this command does
Connects to the Solution Accelerator MCP Server, sends your spec.md and plan.md, and retrieves an architecture blueprint with diagrams.

## Steps (you execute these automatically)

1. Read `spec.md` and `plan.md` from the workspace.
2. Call `blueprint_start` on the Solution Accelerator MCP Server:
...
- If polling exceeds 5 minutes: Report timeout. The Solution Accelerator may be overloaded.
...
### P2 — commands/accelerator.assess.md
...
2. Extract 7 artifact types from the 9 sections by header:
...
7. Display to developer:
   ...
   - If showstoppers: "Fix showstoppers and re-run /accelerator.assess"
   - If no showstoppers: "Assessment passed. You can run /accelerator.generate"
...
### P3 — commands/accelerator.generate.md
...
2. If response is `{ signal: "stop", showstoppers: [...] }`:
   - Report: "Governance gate BLOCKED. Showstoppers remain. Run /accelerator.assess after fixing."
   - STOP. Do not proceed.
...
3. If hashes differ:
   - Call `assemble_blueprint` on the Solution Accelerator MCP Server to regenerate `.json` from `.md`.
...
## Harness (single CI/CD pipeline — infra + build + deploy + promote)
- Generate `.harness/pipeline.yaml` with stages: PRS scan → EvalOps Phase-1 gate → Sign & Attest (cosign + Binary Authorization) → `terraform plan` → approval gate → `terraform apply` (infra) → per-agent `docker build` → deploy to Agent Engine → publish proxy API to Apigee → register the agent as an API Product in API Hub (for that environment).
- Promotion ladder: deploy-nonprod → eval → deploy-preprod (canary 10%) → eval → deploy-prod. Each environment gets its own API Hub registration.
- Multi-region: apply to primary region first, then DR region. NEVER apply without plan approval.
- Auto-rollback if error rate > 1% during canary. Progressive rollout: 10% → 50% → 100%.
...
- Generate a per-environment registration step in the Harness pipeline that registers the agent as an API Product in Apigee API Hub for each environment (non-prod → pre-prod → prod):
...
- Rationale: All deployments go through the Harness pipeline (infra + build + deploy + promote) with approval gates.
...
```json
{
  ...
  "title": "SDLC Accelerators App Blueprint",
  "description": "Machine-readable blueprint derived from app-blueprint.md by assemble_blueprint. Consumed by /accelerator.generate for code generation.",
  "type": "object",
  ...
  "properties": {
    ...
    "blueprint_hash": {
      "type": "string",
      "description": "SHA-256 hash of app-blueprint.md at time of JSON generation. Used by /accelerator.generate to detect edits."
    },
