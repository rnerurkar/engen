...
- **IP boundary:** never introduce meta-skills, STL, or AgentForge-style signed Design Contracts. SDLC Accelerators brownfield = deterministic rules engine + RAG pattern selection + skill-constrained generation. The `design_contract.json` here is a brownfield governance artifact (schema 2.0), NOT an AgentForge signed Design Contract — zero overlap.
