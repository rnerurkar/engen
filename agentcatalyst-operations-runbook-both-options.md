...
| 11. Apigee Proxy + Workload Identity + API Hub A2A Ops | Layer 3 — Generated from app-blueprint.md + Layer 5 — Runtime | Section 2 — `/catalyst.generate` output |
...
Apigee proxy routes, per-agent Workload Identity IAM bindings, and API Hub registration entries are generated by `/catalyst.generate` from `app-blueprint.md` and provisioned during CI/CD. The PRS Security pillar automatically validates Gateway routes, Agent Identity tokens, and Registry entries at build time (see Developer Guide §2 for developer-facing troubleshooting of proxy and identity issues). The health checks below monitor runtime state AFTER deployment.
...
| Failure | Detection | Resolution |
|---|---|---|
| Apigee proxy missing for a tool binding | Deployment smoke test fails | Regenerate: re-run `/catalyst.generate`. Check blueprint §5 for missing binding. |
