...
1. The user opens the **Streamlit web app**, uploads an architecture diagram image (e.g., a PNG of a cloud architecture), and types in a title.
2. The app sends this to the **Orchestrator** — the "traffic controller" that runs the whole show.
...
18. The Orchestrator sends the final documentation — including all HA/DR sections with embedded diagram images — back to the Streamlit app for the user to read. The pattern documentation and the HA/DR sections are shown in **separate expanders** so they're easy to navigate.
19. If the user is happy, they click **"Approve."**
20. The app calls the Orchestrator's `approve_docs` endpoint. The Orchestrator saves the approval in the **CloudSQL database** (the "notebook" that tracks everything).
...
30. The validated code bundle is shown to the user in the Streamlit app.
31. The user reviews the file structure and code. If they're satisfied, they click **"Approve & Publish."**
32. The Orchestrator saves the approval in CloudSQL and immediately kicks off another **background task** — this time to push the code to GitHub. It uses the GitHub REST API to create a new commit with all the generated files organized into folders:
...
34. The Streamlit app enters a **polling loop**: every 3 seconds, it asks the Orchestrator "Are we done yet?" by calling `get_publish_status` with those two tracking IDs.
35. The Orchestrator checks the CloudSQL database and reports back the current status (e.g., "Docs: COMPLETED, Code: IN_PROGRESS").
36. Once both tasks show "COMPLETED," the app displays the final **SharePoint page URL** and **GitHub commit URL** to the user. Done!
