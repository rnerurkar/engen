...
from lib.visualizer import DiagramRenderer
...
class MarkdownToSharePointConverter:
    ...
    *       | Zero or more of previous         | ab* matches "a", "ab", "abb"
    ...
    async def process_markdown_diagrams(self, markdown_text: str, site_assets_uploader_func) -> str:
        """
        Finds ```mermaid blocks, converts to Image via Kroki, uploads to SharePoint,
        and replaces the code block with a SharePoint-hosted <img> tag.
        
        Args:
            markdown_text: The full markdown content.
            site_assets_uploader_func: A callback function (async) that takes (image_bytes, filename)
                                       and returns the SharePoint URL.
        """
        import re
        pattern = r"```mermaid\n(.*?)\n```"
        
        # Strategy: Find all matches, render & upload them, then replace strings.
        matches = list(re.finditer(pattern, markdown_text, flags=re.DOTALL))
        ...
        for idx, match in enumerate(matches):
            ...
            try:
                ...
                png_bytes = self.diagram_renderer.render_mermaid(mermaid_code)
                
                # 2. Upload to SharePoint Assets (via Callback)
                # Generate unique filename
                filename = f"gen_diag_{hash(mermaid_code)}_{random.randint(1000,9999)}.png"
                
                # Await the async uploader
                image_url = await site_assets_uploader_func(png_bytes, filename)
                
                # 3. Replace in text
                replacement = f"![Architecture Diagram]({image_url})"
                current_text = current_text.replace(full_block, replacement)
                
            except Exception as e:
                logger.error(f"Failed to process mermaid diagram {idx}: {e}")
                # Fallback: Keep code block or replace with error text
                continue
...
class SharePointPublisher:
    ...
    async def _get_site_assets_drive_id(self, session: aiohttp.ClientSession) -> Optional[str]:
        """Helper to find the Drive ID specifically for Site Assets."""
        token = await self._ensure_valid_token()
        headers = self._get_headers(token)
        url = f"https://graph.microsoft.com/v1.0/sites/{self.config.site_id}/lists"
        ...
        async with session.get(url, headers=headers) as resp:
            if resp.status == 200:
                ...
                for lst in lists:
                    if lst.get("displayName") == "Site Assets" or lst.get("name") == "SiteAssets":
                        drive_url = f"https://graph.microsoft.com/v1.0/sites/{self.config.site_id}/lists/{lst['id']}/drive"
                        async with session.get(drive_url, headers=headers) as d_resp:
                            if d_resp.status == 200:
                                d_data = await d_resp.json()
                                return d_data.get("id")
        return None

    async def upload_image_to_assets(self, session: aiohttp.ClientSession, image_bytes: bytes, filename: str) -> str:
        """Uploads an image to the 'Site Assets' library and returns its absolute URL."""
        token = await self._ensure_valid_token()
        assets_drive_id = await self._get_site_assets_drive_id(session)
        if not assets_drive_id:
             raise Exception("Could not locate 'Site Assets' library drive.")
        ...
        target_path = f"GeneratedDiagrams/{filename}"
        upload_url = f"https://graph.microsoft.com/v1.0/drives/{assets_drive_id}/root:/{target_path}:/content"
        
        async with session.put(upload_url, headers=headers, data=image_bytes) as resp:
            if resp.status not in (200, 201):
                text = await resp.text()
                raise Exception(f"Image upload failed: {resp.status} - {text}")
    ...
    async def publish_document(
        ...
        try:
            async with aiohttp.ClientSession() as session:
                # Step 0.5: Process Diagrams in Markdown
                # We need to process each section, render mermaid diagrams, invalid images, 
                # upload to SharePoint, and replace links.
                
                async def uploader_wrapper(img_bytes, fname):
                    return await self.upload_image_to_assets(session, img_bytes, fname)
